...
└────────────────────────┬────────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│         阶段2：Elasticsearch结构化验证（验证）                    │
│   ┌─────────────────────────────────────────────────────────┐   │
│   │         四级级联匹配策略                                 │   │
│   │                                                          │   │
│   │  级联1：精确匹配 ───→ 命中？ ──Yes→ 返回标准实体         │   │
│   │    • term查询（keyword精确匹配）         │               │   │
│   │    • match_phrase查询（短语匹配）        │               │   │
│   │                                         No               │   │
│   │                                          │               │   │
│   │  级联2：同义词匹配 ───→ 命中？ ──Yes→ 返回标准实体       │   │
│   │    • 通用名 vs 商品名                    │               │   │
│   │    • 中文名 vs 英文名                    │               │   │
│   │    • 学名 vs 俗称                        │               │   │
│   │    • Orphanet ID（罕见病代码）           │               │   │
...
│   特殊处理：罕见病白名单机制                                      │
│   ┌─────────────────────────────────────────────────────────┐   │
│   │ • 预设罕见病实体库（含Orphanet ID）                      │   │
│   │ • 强制优先执行级联1、级联2（精确+同义词）                │   │
│   │ • 禁止级联3的模糊匹配（防止误判为常见病）                │   │
│   │ • 特殊后缀过滤：['片', '胶囊', '注射液', ...]           │   │
│   └─────────────────────────────────────────────────────────┘   │
└────────────────────────┬────────────────────────────────────────┘
...
└─────────┬──────────┘
          │
          ▼
   shorter = min(query_clean, candidate_clean)
   longer = max(query_clean, candidate_clean)
          │
          ▼
   shorter IN longer?  ──No→  拒绝匹配 ✗
          │
         Yes
          │
          ▼
...
└─────────┬──────────┘
          │
          ▼
   shorter的字符在longer中的顺序一致？  ──No→  拒绝匹配 ✗
          │
         Yes
          │
          ▼
...
└─────────┬──────────┘
          │
          ▼
   overlap_ratio = len(set(shorter) ∩ set(longer)) / len(set(shorter))
          │
          ▼
   overlap_ratio ≥ 85%?  ──No→  拒绝匹配 ✗
          │
         Yes
          │
          ▼
